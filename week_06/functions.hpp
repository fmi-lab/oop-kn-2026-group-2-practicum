...
class Function
{
	...
	/// Тук е важно, да отбележим, че изискваме U, V да имат конструктори по
	/// подразбиране, за да е валиден записа new Pair[new_capacity]. Също,
	/// искаме копиращ оператор '='. След две седмици ще видим как може да
	/// изискваме само конструктор по подразбиране и move-оператор'='.
	void resize()
	{
   ...
   public:
	...
	// тук има edge case, когато data = nullptr, но това е ок, защото тогава
	// capacity = 0, и няма да се опитаме да копираме нищо.
	...
	void addPair(const U &key, const V &value)
	{
		...
		if (index != -1u) { data[index].second = value; }
		else
		{
			append(key, value);
		}
	...
	{
		std::size_t index = find(key);
		if (index != -1u) { return Maybe<V>(data[index].second); }
		else
		{
	...
	{
		if (key.hasValue()) { return getValue(*key); }
		else
		{
			return Maybe<V>();	   // nothing
		}
	...
	// Тук не може да ползваме getValue, защото той връща Maybe<V>, съдържащо
	// КОПИЕ на стойността, а ние искаме РЕФЕРЕНЦИЯ към нея, за да може да се
	// променя. Ако на някой му се чете повече, може да види какво е
	// std::reference_wrapper.
	V &operator[](const U &key) const
	{
		std::size_t index = find(key);
		if (index != -1u) { return data[index].second; }
		else
		{
			exit(1);
		}
	...
	friend Function<A, C> operator*(const Function<A, B> &f,
									const Function<B, C> &g)
	{
		Function<A, C> result;
		for (std::size_t i = 0; i < f.size; ++i)
		{
			...
			// може addPair да приема Maybe<V>, и да добавя само ако има
			// стойност, а ако няма, просто да не прави нищо.
