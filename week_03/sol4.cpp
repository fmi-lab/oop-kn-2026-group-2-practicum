...
class Matrix
{
   ...
   public:
	Matrix(std::size_t r, std::size_t c)
		: rows(r), cols(c), data(new value_type[r * c])
	{
	}
	...
	{
		// забележете, че не заделяме памет преди да сме сигурни, че извикването
		// е валидно
		for (const auto &row : init)
		{
			...
			{
				std::cout
					<< "Error: Inconsistent row sizes in initializer list."
					<< std::endl;
				std::exit(1);
			}
		}
	...
	Matrix &operator=(const Matrix &other)
	{
		if (this != &other)
		{
			if (rows != other.rows || cols != other.cols)
			{
				...
				// често използван трик: деструктора на temp ще освободи старата
				// памет, която сме 'прехвърлили' като отговорност.
				return *this;
			}
			for (std::size_t i = 0; i < rows * cols; i++)
				data[i] = other.data[i];
	...
	void setValue(std::size_t r, std::size_t c, value_type val)
	{
		data[r * cols + c] = val;
	}
	value_type getValue(std::size_t r, std::size_t c) const
	{
	...
	void setRow(std::size_t r, std::initializer_list<value_type> vals)
	{
		if (vals.size() != cols)
		{
			std::cout << "Error: Incorrect number of values for row."
					  << std::endl;
			std::exit(1);
		}
		std::size_t i = 0;
	...
	static Matrix identity(std::size_t n)
	{
		...
		for (std::size_t i = 0; i < n; i++)
			for (std::size_t j = 0; j < n; j++)
				result.at(i, j) =
					(i == j);	  // 1 на диагонала, 0 навсякъде другаде
		return result;
	}
	...
	static Matrix add(const Matrix &m1, const Matrix &m2)
	{
		...
		{
			std::cout << "Error: Incompatible dimensions for addition."
					  << std::endl;
			std::exit(1);
		}
	...
	static Matrix multiply(const Matrix &m1, const Matrix &m2)
	{
		if (m1.cols != m2.rows)
		{
			std::cout << "Error: Incompatible dimensions for multiplication."
					  << std::endl;
			std::exit(1);
		}
