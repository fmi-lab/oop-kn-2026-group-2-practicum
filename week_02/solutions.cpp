...
class Student
{
   ...
   public:
	...
	void AddGrade(float grade, int semester)
	{
		...
		{
			std::cerr << "Grade count exceeded for semester " << semester << "."
					  << std::endl;
			return;
		}
...
// ще счетем, че както се иска в задачата, студентите, които са семестриално
// завършили имат факултетни номера от тип char, а тези, които са редовни - от
// тип int
...
class University
{
   ...
   public:
	...
	University(const char *name, RegularStudent *students, unsigned int count)
		: studentCount(count)
	{
...
int main()
{
	RegularStudent students[2] = {RegularStudent("Alice", 12345),
								  RegularStudent("Bob", 67890)};
	...
	std::cout << "Term Average for Semester 0: " << university.GetTermAverage(0)
			  << std::endl;
	...
	std::cout << "Student Name: " << ptr->GetName()
			  << " Number: " << ptr->GetNumber() << std::endl;
