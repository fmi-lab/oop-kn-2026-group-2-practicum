...
// Указател към функция: <връщан тип> (* <име (optional)>) (<тип на параметър
// 1>, <тип на параметър 2>, ..., <тип на параметър n>) С using:
//      using MyFunctionPointer = <връщан тип> (*) (<тип на параметър 1>, <тип
//      на параметър 2>, ..., <тип на параметър n>) Използваме така:
...
void Sort(int *arr, const size_t size, BoolFunctionPointer f)
{
	for (int i = 0; i < size - 1; i++)
	{
		...
		{
			if (f(arr[i], arr[j])) std::swap(arr[i], arr[j]);
		}
	}
}
...
template <class T>
T ApplyInOrder(const TtoTFunctionPointer<T> *fs, const size_t *arr,
			   const size_t arr_size, int x)
{
...
void test_func1() { std::cout << "a "; }

int test_func2(int a) { return a + 1; }

bool test_func3(int a, int b) { return a > b; }
...
int main()
{
	...
	size_t order1[]		= {2, 0, 1};
	size_t order2[]		= {0, 1, 2};
	std::cout << ApplyInOrder<int>(funcs, order1, 3, 3)
			  << std::endl;		// ((3 - 1) + 1) * 2 = 6
	std::cout << ApplyInOrder<int>(funcs, order2, 3, 3)
			  << std::endl;		// ((3 + 1) * 2) - 1 = 7
