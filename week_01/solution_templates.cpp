...
{
	filtered_size = 0;	   // Добре е да се подсигурим, че filtered_size има
						   // правилна начална стойности
	for (size_t i = 0; i < size; i++)
	{
		if (arr[i] > k) filtered_size++;
	}
...
int main()
{
	...
	std::cout << "Positive elements: " << PositiveElementsCount(test_arr2, 5)
			  << std::endl;
